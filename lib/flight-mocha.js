...
Mocha.interfaces['flight-mocha'] = (function() {
  return function(suite) {
    ...
    suite.on('pre-require', function(context, file, mocha) {
       ...
       * and callback `fn` containing nested suites
       * and/or tests.
       * Load component of Flight before each test.
       *
       * @param componentPath
       * @param fn
       */
       ...
       * and callback `fn` containing nested suites
       * and/or tests.
       * Load component with mixin of Flight before each test.
       *
       * @param componentPath
       * @param fn
       */
      ...
      context.setupComponent = function(fixture, options) {
        var ctx = suites[0].ctx;

        if (ctx.component) {
          ctx.component && ctx.component.teardown();
          ctx.$node.remove();
        }

        ctx.$node = $('<div class="component-root"/>');
        $('body').append(ctx.$node);

        if (fixture instanceof jQuery || typeof fixture === 'string') {
          ctx.$node.append(fixture);
        } else {
          options = fixture;
          fixture = null;
        }
